...
class AccentBtn(Button):
    def __init__(self, text="", bg_color=(0.1, 0.5, 0.95, 1), **kwargs):
        super().__init__(**kwargs)
        self.font_name = FONT_NAME
        self.text = fa(text)
        self.background_normal = ''
...
class ZekrApp(App):
    def build(self):
        ...
        # === هدر ===
        header = Card(orientation='vertical', height=dp(110))
        ...
        # === شمارنده ===
        counter_card = Card(orientation='vertical', height=dp(140))
        ...
        # === نوار پیشرفت ===
        prog_box = BoxLayout(orientation='vertical', size_hint_y=None, height=dp(30), spacing=dp(4))
        ...
        btn_add = StyledBtn("+ ذکر", bg_color=(0.1, 0.55, 0.95, 1))
        btn_add.bind(on_press=self.add_zekr)

        btn_sub = StyledBtn("- کم کردن", bg_color=(0.35, 0.35, 0.5, 1))
        btn_sub.bind(on_press=self.remove_zekr)

        btn_reset = StyledBtn("ریست", bg_color=(0.8, 0.25, 0.35, 1))
        btn_reset.bind(on_press=self.reset_counter)

        btn_target = StyledBtn("تنظیم هدف", bg_color=(0.15, 0.6, 0.45, 1))
        ...
        # === دکمه بانک اذکار ===
        btn_list = AccentBtn("🕌 بانک اذکار مشکل‌گشا", bg_color=(0.55, 0.2, 0.75, 1))
    ...
    def update_time(self, *args):
        try:
            ...
            if HAS_JDATE:
                ...
                weekday_name = SHAMSI_WEEKDAY.get(jnow.strftime("%a"), WEEKDAY_NAMES.get(now.weekday(), ""))
            else:
    ...
    def open_zekr_list(self, *args):
        try:
            ...
            for name in ZEKR_FOLDERS.keys():
                btn = StyledBtn(name, bg_color=(0.2, 0.25, 0.42, 1))
