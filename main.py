...
class TasbihNoorApp(App):
    def build(self):
        ...
        self.btn_target.bind(on_release=self.popup_set_target)
        ...
        self.support_btn.add_widget(support_card)
    ...
    def popup_set_target(self, instance):
        ...
        popup = Popup(title=fa("تنظیم هدف عددی جدید"), content=content, size_hint=(0.85, 0.4))
    ...
    def show_zekr_list(self):
        ...
        for folder_name, _, zekrs in ZEKR_FOLDERS:
            lbl_f = FLabel(text=fa(f"--- {folder_name} ---"), font_size="17sp", color=(0.7, 0.6, 0.9, 1))
            list_layout.add_widget(lbl_f)
            for z in zekrs:
                btn_zekr = StyledBtn(text=fa(z), bg=(0.2, 0.2, 0.35, 1))
                btn_zekr.font_size = "20sp"
